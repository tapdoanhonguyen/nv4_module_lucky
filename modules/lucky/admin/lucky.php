...
if($mod=="duyet"){
    ...
    $db->query('UPDATE ' . TMS_SIEUSERVICE . '_rate SET status = 1 WHERE id = ' . $id);
    $json[] = ['status'=>'OK', 'text'=>'Duyệt thành công!'];
    print_r(json_encode($json[0]));die(); 
}
if($mod=="set_default"){
    $id=$nv_Request->get_string('id', 'get','');
    $db->query('UPDATE ' . NV_LUCKY . ' SET status = 1 WHERE status = 3');
    $db->query('UPDATE ' . NV_LUCKY . ' SET status = 3 WHERE id = ' . $id);
...
$xtpl->assign('OP', $op);
$xtpl->assign('LINK_ADD', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=luckyadd');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['time_begin'] = date('d-m-Y',$view['time_begin']);
        $view['time_end'] = date('d-m-Y',$view['time_end']);
        $view['edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=luckyadd&amp;id=' . $view['id'];
        $view['addprize'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=luckyaddprize&amp;id=' . $view['id'];
