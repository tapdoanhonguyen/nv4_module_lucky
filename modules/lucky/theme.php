...
function nukevn_search_suggestions( $data_content )
{
...
{
    global $module_info, $module_file, $lang_module, $lang_global, $global_array_shops_cat ,$db_config, $db;
    $xtpl = new XTemplate('main_home_cat.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    ...
    if (!empty($array_data)) {
        ...
        foreach ($array_data as $data_row) {
            if ($data_row['num_pro'] > 0) {
                $xtpl->assign('css_cat', $css_cat);
                $xtpl->assign('TITLE_CATALOG', $data_row['title']);
                $xtpl->assign('LINK_CATALOG', $data_row['link']);
                $xtpl->assign('NUM_PRO', $data_row['num_pro']);
                $sql = 'SELECT ' . NV_LANG_DATA . '_description FROM ' . $db_config['prefix'] . '_' . $module_file . '_catalogs';
                $list = $db->query($sql)->fetchAll();
                foreach ($list as $row) {
                    $xtpl->assign('HOME_TEXT', $row['vi_description']);
                }
                if (function_exists('nv_template_' . $viewtype)) {
    ...
    if (!empty($data_content)) {
        ...
        if (!$popup) {
            ...
            if (!empty($data_others)) {
                $html = call_user_func('nv_template_viewgrid_detail', $data_others);
                $xtpl->assign('OTHER', $html);
                $xtpl->parse('main.product_detail.other');
            }
            if (!empty($array_other_view)) {
                $html = call_user_func('nv_template_viewgrid_detail', $array_other_view);
...
function nv_template_viewgrid($array_data, $page = '')
{
    ...
    $xtpl = new XTemplate('viewhomegird.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    $xtpl->assign('LANG', $lang_module);
...
/**
 * nv_template_viewgrid_detail
 *
 * @param mixed $array_data
 * @return
 */
function nv_template_viewgrid_detail($array_data, $page = '')
{
    global $module_info, $lang_module, $lang_global,$global_config, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat;
...
function nv_template_viewcat($data_content, $compare_id, $pages, $sort = 0, $viewtype = 'viewgrid')
{
    ...
    if ($op != 'group' and $global_array_shops_cat[$data_content['id']]['viewcat'] == 'view_home_cat') {
        ...
        $xtpl->assign('CONTENT', call_user_func('nv_template_' . $viewtype . '_detail', $data_content['data'], $pages));
    }
 ...
 * @return
 */
function nv_template_wishlist($array_data, $pages, $viewtype = 'viewgrid_detail')
{
    global $module_info, $lang_module, $lang_global,$global_config, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat;
...
/**
 * nv_template_loadcart_mobile()
 *
 * @param mixed $array_data
 * @return
 */
function nv_template_loadcart_mobile($array_data, $array_products = array())
{
    global $lang_tmp, $module_name, $module_file, $pro_config, $module_info;

    $xtpl = new XTemplate("block.cart_mobile.tpl", NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file);
    $xtpl->assign('LANG', $lang_tmp);
    $xtpl->assign('TEMPLATE', $module_info['template']);
    $xtpl->assign('LINK_VIEW', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=cart");
    $xtpl->assign('WISHLIST', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=wishlist");
